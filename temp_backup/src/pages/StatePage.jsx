...
const StatePage = () => {
  ...
  return (
    <div className="state-page">
      ...
      {/* Gujarat PDF Downloads */}
      {state.id === 'gujarat' && (
        <section className="pdf-downloads section">
          <div className="container">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              ...
              viewport={{ once: true }}
            >
              <h2 className="section-title">Download Travel Guides</h2>
              <p className="section-subtitle">Comprehensive PDF guides for different regions of Gujarat</p>
              
              <div className="pdf-grid">
                {gujaratPDFs.map((pdf, index) => (
                  <motion.a
                    key={index}
                    href={`/soulsailer/info/${pdf.file}`}
                    download
                    className="pdf-card"
                    ...
                    whileHover={{ y: -5 }}
                  >
                    <Download className="pdf-icon" size={24} />
                    <h3>{pdf.name}</h3>
                    <span className="pdf-label">PDF Guide</span>
                  </motion.a>
                ))}
              </div>
            </motion.div>
